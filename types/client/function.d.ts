import {account, acl, aclgroup, player, table, ban, blip,colshape,element,ped,pickup,resource,team,textdisplay,vehicle,xmlnode,textitem,HandleFunction,file,marker,radararea,request,userdata,timer,water} from "mtasa/client/structures";
declare module 'mtasa/client/functions' {
     ...
     * @return * ''false'' if the sound is not a stream.
     * @return * ''nil'' if the sound is invalid.
     */
     ...
     * @return Returns ''false'' if the sound is not playing yet or hasn't buffered in the
     * @return case of streams.
     */
     ...
     * @return The first float is the sound's [http://en.wikipedia.org/wiki/Sampling_rate sample rate], the second one the sound's [http://en.wikipedia.org/wiki/Tempo tempo], and the third one the [http://en.wikipedia.org/wiki/Pitch_%28music%29 pitch] of the sound. The boolean representing whether the sound is reversed or not.
     */
     ...
     * @return {{New feature/item|3.0140|1.4|6443|
     */
     ...
     * @return <syntaxhighlight lang="lua">
     * @return addEventHandler('onClientResourceStart', resourceRoot, function()
     * @return     local mySound = playSound('sound.mp3')
     * @return     setSoundLooped(mySound, true)
     * @return end)
     ...
     * @return If the sound is not 3D, this function will return ''true'' as well, but [[isSoundPanningEnabled]] will always return ''true'' after this (so it has no effect).
     */
     ...
     * @return * '''RemoteEnabled''': ''true'' if remote websites are enabled, ''false'' otherwise
     * @return * '''RemoteJavascript''': ''true'' if Javascript is enabled on remote websites, ''false'' otherwise
     * @return * '''PluginsEnabled''': ''true'' if plugins such as Flash, Silverlight (but not Java) are enabled, ''false'' otherwise. This setting is ''false'' by default.
     */
     ...
     */
    export function guiCreateBrowser(x: number, y: number, width: number, height: number, isLocal: boolean, isTransparent: boolean, isRelative: boolean, parent?: element, ): gui-browser;
     ...
     * @return *'''vehicles:''' if you want the camera to clip on vehicles.
     */
     ...
     * @return '''You should always check to see if this function has returned false.'''
     */
     ...
     * @return Returns a ''true'' if the operation was successful, ''false'' otherwise.
     */
    export function dxDrawWiredSphere(x: number, y: number, z: number, radius: number, theColor: color, fLineWidth: number, iterations: number, ): boolean;
     ...
     * @return If the material is a volume texture, this function will return three ''ints'' representing the width, height and depth.
     */
     ...
     * @return * '''TestMode :''' The current dx test mode. See [[dxSetTestMode]].
     * @return *'''VideoCardName :''' The name of the graphics card.
     * @return *'''VideoCardRAM :''' The installed memory in MB of the graphics card.
     * @return *'''VideoCardPSVersion :''' The maximum pixel shader version of the graphics card.
     * @return *'''VideoCardMaxAnisotropy:''' The maximum anisotropic filtering available. (0-4 which respectively mean: off,2x,4x,8x,16x)
     * @return *'''VideoCardNumRenderTargets:''' The maximum number of simultaneous render targets a shader can use.
     * @return *'''VideoMemoryFreeForMTA :''' The amount of memory in MB available for MTA to use. '''When this gets to zero, [[guiCreateFont]], [[dxCreateFont]] and [[dxCreateRenderTarget]] will fail.'''
     * @return *'''VideoMemoryUsedByFonts :''' The amount of graphic memory in MB used by custom fonts.
     * @return *'''VideoMemoryUsedByTextures :''' The amount of graphic memory in MB used by textures.
     * @return *'''VideoMemoryUsedByRenderTargets :''' The amount of graphic memory in MB used by render targets.
     * @return *'''SettingWindowed :''' The windowed setting. (true/false)
     * @return *'''SettingFullScreenStyle :''' Display style when in full screen mode. (0-2 which respectively mean: Standard, Borderless window, Borderless keep res)
     * @return *'''SettingFXQuality :''' The FX Quality. (0-3)
     * @return *'''SettingDrawDistance :''' The draw distance setting. (0-100)
     * @return *'''SettingVolumetricShadows :''' The volumetric shadows setting. (true/false)
     * @return *'''SettingStreamingVideoMemoryForGTA :''' The usable graphics memory setting. (64-256)
     * @return *'''SettingAnisotropicFiltering:''' The anisotropic filtering setting. (0-4 which respectively mean: off,2x,4x,8x,16x)
     * @return *'''SettingAntiAliasing:''' The anti-aliasing setting. (0-3 which respectively mean: off,1x,2x,3x)
     * @return *'''SettingHeatHaze:''' The heat haze setting. (true/false)
     * @return *'''SettingGrassEffect:''' The grass effect setting. (true/false)
     * @return *'''Setting32BitColor:''' The color depth of the screen. (false is 16bit, true is 32bit)
     * @return *'''SettingHUDMatchAspectRatio:''' The hud match aspect ratio setting (true/false)
     * @return *'''SettingAspectRatio:''' The aspect ratio setting ("auto", "4:3", "16:10", "16:9")
     * @return *'''SettingFOV:''' The FOV setting
     * @return {{New feature/item|3.0154|1.5.3|11199|
     * @return *'''SettingHighDetailVehicles:''' High detail vehicles setting (true/false).
     * @return }}
     * @return {{New feature/item|3.0160|1.5.8||
     * @return *'''SettingHighDetailPeds:''' High detail peds setting (true/false).
     * @return }}
     * @return *'''AllowScreenUpload :''' The allows screen uploads setting. (true/false)
     * @return *'''DepthBufferFormat:''' The format of the shader readable depth buffer, or 'unknown' if not available
     * @return *'''UsingDepthBuffer:''' ''true'' if the depth buffer is used, ''false'' otherwise
     */
     ...
     * @return *'''float:''' aspect ratio set by [[dxSetAspectRatioAdjustmentEnabled]]
     */
     ...
     * @return Returns ''true'' if the function was successful, ''false'' otherwise.
     * @return '''Note:''' [[updateElementRpHAnim]] must be called after this function to apply bone rotation.
     */
     ...
     * @return Do not rely on the model numbers returned being consistent across multiple clients or multiple runs of resources. There is no guarantee for the order of the numbers or that the same numbers will always correspond to the same element type. Any patterns are coincidental
     */
     ...
     * @return {{Tip|If you want to synchronize animations, you can check [[https://drive.google.com/open?id=1L2NkQYuLS0YdoHECvxVRMdPBbEgaYUfH ifp_demo]] resource}}
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the model was successfully restored, ''false'' or ''nil'' if it failed for some reason.
     */
     ...
     * @return *2 numbers if '''chat_scale''' was entered
     * @return *1 number if any other CVar was specified
     * @return *a table of all CVar values, if CVar was not specified
     * @return *''false'' if an invalid CVar was specified
     */
     ...
     * @return * '''"none"'''            // cursor has no image
     * @return * '''"arrow"'''           // default cursor
     * @return * '''"sizing_ns"'''       // N-S (up-down) sizing cursor
     * @return * '''"sizing_ew"'''       // E-W (left-right) sizing cursor
     * @return * '''"sizing_nwse"'''     // NW-SE diagonal sizing cursor
     * @return * '''"sizing_nesw"'''     // NE-SW diagonal sizing cursor
     * @return * '''"sizing_eswe"'''     // ES-WE horizontal sizing cursor
     * @return * '''"move"'''            // move cursor
     * @return * '''"container_drag"'''  // drag container cursor (note: not in use)
     * @return * '''"segment_moving"'''  // segment moving cursor (note: not in use)
     * @return * '''"segment_sizing"'''  // segment sizing cursor (note: not in use)
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return *'''string''' A string containing the name of the element's current font, or false if the gui element passed to the function is invalid.
     * @return *'''element ''' The custom [[GUI font]] that is used, or nil otherwise
     */
     ...
     * @return * '''"allow_binds":''' binds are enabled, hence using a key such as t in an editbox will still activate the chatbox
     * @return * '''"no_binds":''' binds are disabled, hence using a key such as t in an editbox will not activate the chatbox
     * @return * '''"no_binds_when_editing":''' binds are always enabled except when an editable editbox or memo has input focus
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns a string containing the requested element's text, or false if the gui element passed to the function is invalid.
     */
     ...
     * @return table = {
     * @return     [1] = {
     * @return         ["column"], -- has the first selected item's column ID
     * @return         ["row"] -- has the first selected item's row ID
     * @return     },
     * @return     [2] = {
     * @return         ["column"],-- has the second selected item's column ID
     * @return         ["row"] -- has the second selected item's row ID
     * @return     },
     * @return     ...
     * @return }
     * @return </syntaxhighlight>
     * @return if everything was successful or ''false'' if invalid arguments were passed.
     */
     ...
     * @return {{GUI_gridList_selection_modes_IDs}}
     */
     ...
     */
    export function guiCreateMemo(x: number, y: number, width: number, height: number, text: string, relative: boolean, parent?: element, ): gui-memo;
     ...
     */
    export function guiCreateScrollBar(x: number, y: number, width: number, height: number, horizontal: boolean, relative: boolean, element: gui, ): gui-scrollbar;
     ...
     * @return * ''false'' if the object is not breakable, or a wrong object was given.
     */
     ...
     * @return * ''false'' if invalid arguments were passed.
     * @return * ''-1'' if object was never streamed in.
     */
     ...
     * @return * ''false'' if the object is not breakable.
     */
     ...
     * @return * ''false'' if it can't or if invalid arguments are passed. 
     */
     ...
     * @return * ''false'' if the object is not moving.
     */
     ...
     * @return <section name="Client-side example" class="client" show="true">
     * @return <syntaxhighlight lang="lua">
     * @return x, y, z = getElementPosition(localPlayer)
     * @return rotX, rotY, rotZ = getElementRotation(localPlayer)
     * @return CJ = createPed(0, x, y, z)
     * @return CJ2 = createPed(0, x, y, z)
     * @return copcar = createVehicle(597, x, y+5, z+2)
     * @return copcar2 = createVehicle(597, x+2, y+10, z+4)
     * @return warpPedIntoVehicle(CJ2, copcar2, 0)
     * @return warpPedIntoVehicle(CJ, copcar, 0)
     * @return function onGetCJAnalogControlState()
     * @return setPedAnalogControlState(CJ, "accelerate", 0.90)
     * @return if getPedAnalogControlState(CJ, "accelerate", 0.90) then
     * @return setPedAnalogControlState(CJ2, "brake_reverse", 0.90)
     * @return end
     * @return end
     * @return addEventHandler("onClientRender", getRootElement(), onGetCJAnalogControlState)
     * @return </syntaxhighlight>
     * @return </section>
     * @return <br /><br />
     * @return * This example was created by '''Hydra'''.
     */
     ...
     * @return {{New feature/item|3.0160|1.5.7|20450|<syntaxhighlight lang="lua"> string, string, int, int, bool, bool, bool, int, bool getPedAnimation ( ped thePed )</syntaxhighlight>
     * @return '''The function now returns 9 values in the same order as required by setPedAnimation:'''
     * @return <syntaxhighlight lang="lua">string anim, string block, int time, bool loop, bool updatePosition, bool interruptable, bool freezeLastFrame, int blendTime, bool restoreTaskOnAnimEnd</syntaxhighlight>
     * @return }}
     */
     ...
     * @return <br>
     * @return {{New feature|3.0110|1.1|
     * @return Returns between 1 and 4 strings. The first string contains the name of the most complex task, with simpler sub-tasks being named in the following strings. See [[list of player tasks]] for valid strings. Returns ''false'' if invalid arguments are specified or if there is no task of the type specified.
     * @return }}
     */
     ...
     * @return Returns ''true'' if the function was successful, ''false'' otherwise.
     * @return When this function returns ''true'', the client will ask server for permission to enter a vehicle. Actually entering can still fail in the following cases
     * @return *The function is used on a ped, but another client is not on version '''1.5.8 r20740''' or newer.
     * @return *Another player or ped is already entering, exiting or jacking the same vehicle and seat.
     * @return *Movement input or damage can interrupt the task. Use [[getPedTask]] to monitor what the ped is doing.
     * @return This function returns ''false'' in the following cases
     * @return *Invalid arguments were parsed.
     * @return *The function is used on a ped, but the server is not on version '''1.5.8 r20740''' or newer.
     * @return *Time passed since last enter/exit for this ped is less than 1500 ms.
     * @return *[[onClientVehicleStartEnter]] was cancelled by a script.
     * @return *The ped has an active TASK_PRIMARY [[task]]. Use [[getPedTask]] to monitor what the ped is doing.
     */
     ...
     * @return Returns ''true'' if the function was successful, ''false'' otherwise.
     * @return When this function returns ''true'', the client will ask server for permission to exit a vehicle. Exiting can still fail in the following cases
     * @return *The function is used on a ped, but another client is not on version '''1.5.8 r20740''' or newer.
     * @return This function returns ''false'' in the following cases
     * @return *Invalid arguments were parsed.
     * @return *The function is used on a ped but the server is not on version '''1.5.8 r20740''' or newer.
     * @return *Time passed since last enter/exit for this ped is less than 1500 ms.
     * @return *The ped is getting jacked.
     */
     ...
     * @return ** ''minX, minY'' represent the world coordinates ''-3000, 3000'' (upper-left corner of the world map).
     * @return ** ''maxX, maxY'' represent the world coordinates ''3000, -3000'' (lower-right corner of the world map).
     * @return ** Negative values may be returned if these coordinates are off screen.
     * @return * If the map is not showing, a ''false'' boolean value is returned.
     */
     ...
     * @return * Grenades will explode when it hits 0
     * @return * Teargas may be a duration timer
     * @return * Both types of rockets will explode when it hits 0
     * @return * Satchels restarts so I do not think it does anything
     */
     ...
     * @return {{New feature/item|3.0141|1.4.0|6990|If the projectile is a satchel charge, returns the [[element]] at which it is glued to (or ''nil'' if it isn't glued to any).}}
     */
     ...
     * @return {| class="wikitable" style="cellpadding: 10px;"
     * @return |-
     * @return ! Property || Values and description
     * @return |-
     * @return | <code>readingLayout</code> ||
     * @return     {| class="prettytable"
     * @return     |-
     * @return     | <code>"ltr"</code> || Left to right (English)
     * @return     |-
     * @return     | <code>"rtl"</code> || Right to left (Arabic, Hebrew)
     * @return     |-
     * @return     | <code>"ttb-rtl-ltr"</code> || Either read vertically from top to bottom with columns going from right to left, or read in horizontal rows from left to right, as for the Japanese (Japan) locale.
     * @return     |-
     * @return     | <code>"ttb-ltr"</code> || Read vertically from top to bottom with columns going from left to right, as for the Mongolian (Mongolian) locale.
     * @return     |}
     * @return |}
     */
     ...
     * }}
     * @return Returns a [[table]] with the following entries:
     * @return *'''code :''' The language code ''(eg. "en_US" for "English (United States)" or "ar" for "Arabic")''.
     * @return *'''name :''' The name of the language ''(eg. "English (United States)" or "Arabic")''.
     */
     ...
     * @return This is a command to get the position of the player's vehicle dummy position specified as an argument.
     * @return <syntaxhighlight lang="lua">
     * @return function getDummyPosition(cmd, dummy)
     * @return     if (not dummy) then
     * @return         return false
     * @return     end
     * @return     local veh = getPedOccupiedVehicle(localPlayer)
     * @return     if (not veh) then
     * @return         outputChatBox("You should be in a vehicle to use this command", 255, 25, 25)
     * @return         return false
     * @return     end
     * @return     local x, y, z = getVehicleDummyPosition(veh, dummy)
     * @return     outputChatBox("X: "..x..", Y: "..y..", Z: "..z, 0, 255, 0)
     * @return end
     * @return addCommandHandler("getdummy", getDummyPosition)
     * @return </syntaxhighlight>
     */
     ...
     * @return * '''0:''' Normal friction
     * @return * '''1:''' Slip with acceleration (only for driving wheels)
     * @return * '''2:''' Slip without acceleration
     * @return * '''3:''' Locked wheel (on brake on handbrake).
     */
     ...
     * @return ** '''reloading''': the weapon is reloading.
     * @return ** '''firing''': the weapon is constantly shooting (unless any shooting blocking flags are set) according to its assigned firing rate.
     * @return ** '''ready''': the weapon is idle.
     * @return * ''false'' if an error occured or the [[Element/Weapon|weapon]] is invalid.
     */
     ...
     * @return **''[[nil]]'' if the weapon is in rotation based targeting.
     * @return **3 [[float|floats]] if the weapon is firing at a fixed point.
     * @return **an [[element]] if the weapon is firing an entity.
     * @return * Returns ''false'' if the weapon element is not valid.
     */
     ...
     * @return ''Western X position, Eastern X position, Southern Y position, Northern Y position,, false when invalid garageID was provided.''
     */
     ...
     * @return The other values are only filled if there is a collision, they contain ''nil'' otherwise
     * @return *'''hitX, hitY, hitZ:''' collision position
     * @return *'''hitElement:''' the MTA element hit if any, ''nil'' otherwise
     * @return *'''normalX, normalY, normalZ:''' the normal of the surface hit
     * @return *'''material:''' an integer representing the [[Material IDs|GTASA material ID]] of the surface hit when applicable (world, objects)
     * @return *'''lighting:''' a float between 0 (fully dark) and 1 (bright) representing the amount of light that the hit building surface will transfer to peds or vehicles that are in contact with it. The value can be affected by the game time of day, usually with a lower (darker) value being returned during the night.
     * @return *'''piece:''' an integer representing the part of the element hit if hitElement is a vehicle or a ped/player, ''0'' otherwise. 
     * @return **For a ped/player, piece represents the body part hit:
     * @return {{BodyParts}}
     * @return **For vehicles, piece represents the vehicle part hit:
     * @return {{VehicleParts}}
     * @return *'''worldModelID:''' If includeWorldModelInformation was set to ''true'' and a world model was hit, this will contain the model ID.
     * @return *'''worldModelPositionX,Y,Z:''' If worldModelID is set, this will contain the world model position.
     * @return *'''worldModelRotationX,Y,Z:''' If worldModelID is set, this will contain the world model rotation.
     * @return *'''worldLODModelID:''' If worldModelID is set, this will contain the LOD model ID if applicable.
     */
...
int,--,worldLODModelID]>;
     ...
     * @return {{New feature|3|1.0|
     * @return * Returns ''false'' if the camera is in Fixed mode and has no target.
     * @return }}
     */
     ...
     * @return {{New items|5.0157|1.5.7-9.20477|
     */
     ...
     * @return *'''tickStart:''' A number estimating how many ticks until the data transfer starts (Negative means the transfer has already started)
     * @return *'''tickEnd:''' A number estimating how many ticks until the data transfer completes
     * @return *'''totalSize:''' A number representing how many bytes in total this transfer will transfer
     * @return *'''percentComplete:''' A number between 0-100 saying how much is done
     */
     ...
     * @return * ''false'' if invalid parameters were passed.
     */
     ...
     * @return * '''bytesReceived''' - Total number of bytes received since the connection was started
     * @return * '''bytesSent''' - Total number of bytes sent since the connection was started
     * @return * '''packetsReceived''' - Total number of packets received since the connection was started
     * @return * '''packetsSent''' - Total number of packets sent since the connection was started
     * @return * '''packetlossTotal''' - (0-100) Total packet loss percentage of sent data, since the connection was started
     * @return * '''packetlossLastSecond''' - (0-100) Packet loss percentage of sent data, during the previous second
     * @return * '''messagesInSendBuffer'''
     * @return * '''messagesInResendBuffer''' - Number of packets queued to be resent (due to packet loss)
     ...
     */
    export function setWeaponProperty(theWeapon: weapon, strProperty: string, theValue: value, ): boolean;
