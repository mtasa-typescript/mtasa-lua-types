 ...
 * handler's 'attachedTo' also to resourceRoot.}}
 * -->__NOTOC__
 * This function triggers an event previously registered on a client. This is the primary means of
 * passing information between the server and the client. Clients have a similar [[triggerServerEvent]]
 * function that can do the reverse. You can treat this function as if it was an asynchronous function
 * call, using [[triggerServerEvent]] to pass back any returned information if necessary.
 ...
 * as they do not necessarily have a valid representation on the client.
 * Events are sent reliably, so clients will receive them, but there may be (but shouldn't be) a
 ...
 * {{Note|To save client CPU, you should avoid setting '''sourceElement''' to the [[root element]] where
 * possible. Using [[GetThisResource|resourceRoot]] is usually sufficient if the event is handled by
 * the same resource on the client.}}  {{OptionalArg}}
...
/**
 * This function checks if a pickup is currently spawned (is visible and can be picked up) or not
 * (a player picked it up recently).
 * @see {@link https://wiki.multitheftauto.com/wiki/isPickupSpawned|MTASA Wiki}
 * @param thePickup the pickup you want to check.
 * @return Returns ''true'' if the pickup is spawned, ''false'' if it's not spawned or an invalid
 * pickup was specified.
 */
 ...
 * @param bSave Set to true to make the setting permanent, or false for use only until the next
 * server restart.
 * @default false
 ...
 * @default 255
 * @param visibleTo : This defines which elements can see the marker. Defaults to visible to
 * everyone. See visibility.
 * @default getRootElement(
