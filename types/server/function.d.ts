import {account, acl, aclgroup, player, table, ban, blip,colshape,element,ped,pickup,resource,team,textdisplay,vehicle,xmlnode,textitem,HandleFunction,file,marker,radararea,request,userdata,timer,water} from "mtasa/server/structures";
declare module 'mtasa/server/functions' {
     ...
     * @return Returns ''true'' if successfully destroyed and ''false'' if it could not be deleted (ie. it's not valid).
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the ACL group was successfully deleted, ''false'' if it could not be deleted for some reason (ie. invalid argument).
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns the ACL group if it could be found. Returns false/nil if it did not exist or failed for some reason.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' or ''false'' if the ACL gives access or not to the given function. Returns ''nil'' if it failed for some reason, e.g. an invalid ACL was specified or the right specified does not exist in the ACL.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the ACL could be successfully added to the ACL group, ''false''/''nil'' if either of the elements are invalid, the ACL is already in that group or if something else goes wrong.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the object was successfully added to the ACL, ''false'' if it already existed in the list.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns the name of the given ACL group as a string if successful, otherwise ''false'' or ''nil'' if the aclGroup is invalid or it fails for some other reason.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns a table of the ACL elements in the given ACL group. This table might be empty. Returns ''false'' or ''nil'' if theGroup is invalid or it fails for some other reason.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns a table of strings in the given ACL group. This table might be empty. Returns ''false'' or ''nil'' if theGroup is invalid or it fails for some other reason.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the object existed in the ACL and could be remoevd, ''false'' if it could not be removed for some reason, ie. it did not exist in the given ACL group.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the XML was successfully reloaded from the file, ''false'' or ''nil'' if the XML was invalid, didn't exist or could not be loaded for some other reason.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the given right was successfully removed from the given ACL, ''false'' or ''nil'' if it could not be removed for some reason, ie. it didn't exist in the ACL.
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns ''true'' if the ACL was successfully changed, ''false'' or ''nil'' if it could not be saved for some reason, ie. file in use.
     */
     ...
     * @return * Returns '''false''' if invalid arguments were specified or if there was no banning time specified for the [[ban]].
     */
     ...
     * @return * Returns '''false''' if invalid arguments are specified or if there was no unbanning time specified for the [[ban]].
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns a table of all the currently loaded [[modules]]. If no modules are loaded, the table will be empty.
     */
     ...
     * @return *'''version''': Module version in format X.XX
     * @return *'''name''': Module name
     * @return *'''author''': Module author
     * @return If invalid name for module is passed, it will return ''false''.
     */
     ...
     * @return '''Note:''' this will fail but return true if
     * @return 1) the ped is crouched and moving
     * @return 2) the ped is using a weapon without clip ammo (or minigun/flamethrower/fire
     * @return extinguisher)
     * @return 3) the ped is using his weapon (shooting/aiming)
     * @return 4) the ped moved while crouching recently
     * @return Due to these circumstances causing problems with this function
     */
     ...
     * @return * '''DetectedAC:''' A string containing a comma separated list of [[Anti-cheat_guide|anti-cheat]] codes the player has triggered.
     * @return *'''d3d9Size:''' A number representing the file size of any custom d3d9.dll the player may have installed.
     * @return *'''d3d9MD5:''' A string containing the MD5 of any custom d3d9.dll the player may have installed.
     * @return *'''d3d9SHA256:''' A string containing the SHA256 of any custom d3d9.dll the player may have installed.
     * @return <!--
     */
     ...
     * @return If successful, returns the UNIX timestamp when the resource was last started, or the string "never" if the resource has not been started yet, otherwise false. Use in conjunction with [[getRealTime]] in order to retrieve detailed information. 
     * @return }}
     * @return {{Deprecated_feature|3|1.0|
     * @return Returns a string with the time and date, or false if the resource does not exist.
     * @return }}
     */
     ...
     * @return If successful, returns the UNIX timestamp when the resource was loaded, otherwise false. Use in conjunction with [[getRealTime]] in order to retrieve detailed information. 
     * @return }}
     * @return {{Deprecated_feature|3|1.0|
     * @return If successful, returns a string with the date and time that the resource was last loaded into memory (for example when the server started, or when the resource was changed and reloaded). Returns ''false'' on failure.
     * @return An example string is "Fri Mar 28 13:51:04 2008".
     * @return }}
     */
     ...
     * @return <!-- Make this descriptive. Explain what cases will return false. If you're unsure, add a tag to it so we can check -->
     * @return Returns the current server password as a string if it has a password, if not it returns ''nil''.
     */
     ...
     * @return *the list of global setting name/value pairs if "." is passed as a setting name,
     * @return *the list of resource settings if a resource name followed by a "." is passed,
     * @return *the list of the script's resource settings if an empty string is passed.
     * @return It returns ''false'' if the specified setting or settings group doesn't exist, or if the settings group you are trying to retrieve doesn't have any public or protected settings.
     */
     ...
     * @return {
     * @return     { colname1=value1, colname2=value2, ... },
     * @return     { colname1=value3, colname2=value4, ... },
     * @return     ...
     * @return }
     * @return </syntaxhighlight>
     * @return A subsequent table represents the next row.
     */
     ...
     * @return *''false:'' Returns false if the query string contained an error, the connection has been lost or the query handle is incorrect. This automatically frees the query handle, so you do not have to call [[dbFree]].
     * @return ** This also returns two extra values: (See the example on how the retrieve them)
     ...
     * @return ** This also returns extra values (only when multipleResults is set to true):
     * @return ***''int:'' number of affected rows
     * @return ***''int:'' last insert id
     * @return The table is of the format:
     * @return <syntaxhighlight lang="lua">
     * @return {
     * @return     { colname1=value1, colname2=value2, ... },
     * @return     { colname1=value3, colname2=value4, ... },
     ...
     * @return </syntaxhighlight>
     * @return A subsequent table represents the next row.
     * @return {{Note|If a column contained a number it is returned as a number, this includes things which were stored as string but are numbers such as "1" would be returned as 1.}}
     */
     ...
     * @return If the setting name is ''serverip'', may return the string ''"auto"'' on local servers.
     */
